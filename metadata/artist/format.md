...
frontend 向け生成物は、単なる辞書や配列ではなく、トップレベルに共通 envelope を持つ。
...
`artistctl` 単体で決める値ではなく、`tools/build.sh` などの上位 orchestration が生成して `minify` に渡す。
...
```jsonc
{
  ...
  "data": [
    { "key": "栞葉るり", "liverId": "ruri-shioriha" },
    { "key": "しおりはるり", "liverId": "ruri-shioriha" },
    { "key": "Shioriha Ruri", "liverId": "ruri-shioriha" },
    { "key": "倉持めると", "liverId": "meruto-kuramochi" },
    { "key": "くらもちめると", "liverId": "meruto-kuramochi" },
    { "key": "Kuramochi Meruto", "liverId": "meruto-kuramochi" },
    // aliasは表示の優先度を下げたいので, aliasかどうかを判断できるようにしておく
    { "key": "めるち", "liverId": "meruto-kuramochi", "isAlias": true },
  ],
}
```
