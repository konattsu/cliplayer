...
最低限、frontend が比較可能な `indexBuildId` と、
互換性確認に使える binary format version を読める必要がある。

注意:

- min JSON では実装済みだが、検索 index 側は未統一の可能性がある
- frontend 設計としては必須にする
- Rust tools 側で後追い実装する
...
type PlaybackAvailability =
  | { kind: "playable" }
  | { kind: "blocked-by-tag"; tagIds: string[] }
  | { kind: "not-embeddable" }
  | { kind: "missing-video" }
  | { kind: "missing-clip" }
  | { kind: "unavailable-on-youtube" };
```
...
type PlayQueue = {
  ...
  currentIndex: number;
  source:
    | { kind: "single-clip"; clipUuid: string }
    | { kind: "search-page"; queryKey: string }
...
- `version` 不一致なら破棄
- `dataBuildId` 不一致なら queue を再検証する
...
1. Rust tools 側で `schemaVersion`, `dataBuildId`, `indexBuildId` の方針を固める
