...
impl YouTubeApi {
    ...
    pub async fn run(
        ...
        match self.fetch_process().await {
            Ok(_) => {
                // 全ての動画idに対してfetch出来たとき
                tracing::info!("Successfully fetched video details.");
            }
            Err(e) => {
                // 全ての動画idに対してfetch出来なかったときはReturn Err
...
enum state {
    ...
    Error(crate::fetcher::YouTubeApiError),
}
...
mod tests {
    ...
    #[test]
    fn test_youtube_api_next_video_id_batch() {
        let mut api = YouTubeApi::new(crate::model::YouTubeApiKey::dummy_api_key());
        api.pending_ids = vec![
        ...
        // let batch = tokio_test::block_on(api.next_video_id_batch());
        // assert_eq!(batch.len(), yt_api_const::MAX_RESULTS as usize);
        // assert_eq!(batch[0], crate::model::VideoId::new("id1"));
        // assert_eq!(batch[1], crate::model::VideoId::new("id2"));
        // assert_eq!(batch[2], crate::model::VideoId::new("id3"));
    }
}
