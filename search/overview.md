...
検索 index build には frontend から比較可能な `index_build_id` を持たせる。
...
- frontend は index load 時に `index_build_id` を読める必要がある
- cursor token の再利用可否は `index_build_id`, query, sort で判定する
- `index_build_id` は min JSON 側の `dataBuildId` とは別物として扱う
  - s2 と s3 は生成頻度が異なるため
