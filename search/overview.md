...
検索エンジンの実装設計は [`engine/design.md`](./engine/design.md) を参照。
物理 format の詳細は [`binary_schema.md`](./binary_schema.md) を参照。
入力データ全体の位置づけは [`data_flow.md`](../docs/data_flow.md) を参照。
...
また frontend が cursor や永続化済み queue の不整合を検出できるよう、
検索 index build には frontend から比較可能な `dataset_build_id` を持たせる。
...
`dataset_build_id`, `query_fingerprint`, sort 情報, seek key を持つ。
...
- frontend は index load 時に `dataset_build_id` を読める必要がある
- cursor token の再利用可否は `dataset_build_id`, query, sort で判定する
- `dataset_build_id` は min JSON 側の `datasetBuildId` と同じ build identity を表す
  - min JSON 群と検索 index は同じ source-of-truth から同時生成されるため

## 8. Cursor と URL の方針

cursor についての前提は次のとおり。

- cursor は同一 `dataset_build_id` に対してのみ有効
...
engine 実装本体の設計は [`engine/design.md`](./engine/design.md) を参照。
