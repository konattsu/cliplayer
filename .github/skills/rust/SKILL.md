...
IMPORTANT: This project is currently under development, so please keep changes to a minimum and do not worry about compatibility at all.
...
- Implement traits to abstract services or external dependencies.
  - But avoid overusing traits for abstraction. Consider the "Rise of Worse is Better" philosophy.
...
- Must follow the Rust Style Guide and use `rustfmt` for automatic formatting.
  - Don't hesitate to run `cargo fmt` to ensure consistent formatting.
...
- functions for tests(i.e. with `#[test]`) should be named `test_*`.
